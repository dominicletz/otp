...
void wxGraphicsContext_DrawLines(WxeApp *app, wxeMemEnv *memenv, wxeCommand& Ecmd)
{
 wxPolygonFillMode fillStyle=wxODDEVEN_RULE;
  ...
  wxGraphicsContext *This;
  This = (wxGraphicsContext *) memenv->getPtr(env, argv[0], "This");
  wxPoint2DDouble * points, * points_ptr;
  unsigned pointsLen;
  ...
  points = (wxPoint2DDouble *) enif_alloc(sizeof(wxPoint2DDouble)*pointsLen);
  points_ptr = points;
  double x, y;
  pointsTail = argv[1];
...
void wxControlWithItems_appendStrings_2(WxeApp *app, wxeMemEnv *memenv, wxeCommand& Ecmd)
{
  ...
  wxeErlTerm ** clientsData;
  wxeErlTerm ** clientsData_ptr;
  ...
  clientsData = (wxeErlTerm **) enif_alloc(sizeof(wxeErlTerm *)*clientsDataLen);
  clientsData_ptr = clientsData;
  ...
  while(!enif_is_empty_list(env, clientsDataTail)) {
    ...
    *clientsData_ptr = new wxeErlTerm(argv[2]);
    clientsData_ptr++;
  };
  if(!This) throw wxe_badarg("This");
  int Result = This->Append(items,(wxClientData **) clientsData);
  enif_free(clientsData);
  wxeReturn rt = wxeReturn(memenv, Ecmd.caller, true);
  rt.send(  rt.make_int(Result));
...
void wxControlWithItems_insertStrings_3(WxeApp *app, wxeMemEnv *memenv, wxeCommand& Ecmd)
{
  ...
  while(!enif_is_empty_list(env, clientsDataTail)) {
    if(!enif_get_list_cell(env, clientsDataTail, &clientsDataHead, &clientsDataTail)) Badarg("clientsData");
    *clientsData_ptr = new wxeErlTerm(argv[3]);
    clientsData_ptr++;
  };
  if(!This) throw wxe_badarg("This");
  int Result = This->Insert(items,pos,(wxClientData **) clientsData);
  enif_free(clientsData);
  wxeReturn rt = wxeReturn(memenv, Ecmd.caller, true);
  rt.send(  rt.make_int(Result));
...
void wxStatusBar_SetFieldsCount(WxeApp *app, wxeMemEnv *memenv, wxeCommand& Ecmd)
{
  ...
  widths = (int *) enif_alloc(sizeof(int)*widthsLen);
  widths_ptr = widths;
  widthsTail = tpl[1];
...
void wxStatusBar_SetStatusWidths(WxeApp *app, wxeMemEnv *memenv, wxeCommand& Ecmd)
{
  ...
  widths_field = (int *) enif_alloc(sizeof(int)*widths_fieldLen);
  widths_field_ptr = widths_field;
  ...
  while(!enif_is_empty_list(env, widths_fieldTail)) {
    ...
    *widths_field_ptr = (int) widths_field_tmp;
    widths_field_ptr++;
  };
  if(!This) throw wxe_badarg("This");
  This->SetStatusWidths(widths_fieldLen,widths_field);
  enif_free(widths_field);
...
void wxStatusBar_SetStatusStyles(WxeApp *app, wxeMemEnv *memenv, wxeCommand& Ecmd)
{
  ...
  styles = (int *) enif_alloc(sizeof(int)*stylesLen);
  styles_ptr = styles;
  stylesTail = argv[1];
