...
enif_get("GLintptr")   -> "enif_get_int64";
enif_get("GLsizeiptr") -> "enif_get_int64";
enif_get("GLsync")     -> "egl_get_ptr";
enif_get("GLhandleARB") -> "enif_get_uint64".

%% cast_to_type("GLintptr")    -> "(uint64_t)";
%% cast_to_type("GLsizeiptr")  -> "(uint64_t)";
...
apply_func(F=#func{name=Name,id=_Id,type=Type,params=As0}) ->
    ...
    {As2, _} = lists:mapfoldl(fun decode_var/2, 0, As1),
...
declare_var(P=#arg{name=Name, in=true, type=#type{name=T,single={Comp, Sz}}},
  ...
  when (Sz < 17 orelse Sz =:= undefined) andalso (Comp =:= tuple orelse Comp =:= list) ->
    ...
    {P,Argc+1};
declare_var(P=#arg{name=Name, in=true, type=#type{name=T, single=Single}}, Argc) ->
    if Single =:= list; element(1,Single) =:= list ->
            w("  int ~s_free = 0;\n",[Name]);
       true -> ignore
    end,
    w("  ~s *~s;\n",[T,Name]),
...
declare_var(P=#arg{name=Name, in=false, type=#type{name=T, single={Single,Sz}}}, Argc)
  when Single =:= list; Single =:= tuple ->
    ...
    {P,Argc+1};
declare_var(P=#arg{name=Name, in=false, type=#type{name=T, single={list,Sz,_}}}, Argc) ->
    case is_integer(Sz) of
        true -> exit(foo), w("  ~s ~s[~w];\n", [T, Name, Sz]);
        false ->
            w("  ~s * ~s;\n", [T, Name]),
            w("  ERL_NIF_TERM *~s_ts;\n", [Name])
    end,
...
decode_var(P=#arg{name=Name,type=#type{base=string,ref={pointer,2},mod=[const]}},Argc) ->
    w("  if(!enif_inspect_binary(env, argv[~w], &~s_all)) Badarg(~w,\"~s\");\n",
      [Argc, Name,?OP,Name]),
    w("  ~s = (GLchar **) enif_alloc(sizeof(GLchar *) * (int) argv[~w]);\n",[Name, Argc-1]),
    w("  ~s_ptr = (GLchar *) ~s_all.data;\n", [Name,Name]),
    w("  for(i=0; i < (int) argv[~w]; i++) {"
      "     ~s[i] = (GLchar *) ~s_ptr;\n"
      "     ~s_ptr += 1+strlen(~s_ptr);\n"
      "  }\n", [Argc-1,Name, Name, Name, Name]),
    store_free(Name),
    {P,Argc+1};
...
decode_var(P=#arg{name=Name, in=true, alt=Alt,
  ...
  when Base =:= int; Base =:= float; Base =:= guard_int ->
    ...
    case Alt of
        list_binary ->
            w(" {\n", []),
            w("    if(!enif_is_binary(env, argv[~w])) Badarg(~w, \"~s\");~n",[Argc,?OP,Name]),
    ...
    Len = case Alt of
              {length, Var} -> Var;
              list_binary ->
                  w("    enif_get_list_length(env, argv[~w], &~s_len);\n",[Argc,Name]),
                  Name ++ "_len";
              _ ->
                  w("    unsigned int ~s_len;\n"
    ...
    w("    ~s_free = 1;\n",[Name]),
    w("    ~s = (~s *) enif_alloc(~s * sizeof(~s));\n",[Name,Type, Len, Type]),
    w("    ~s_l = argv[~w];\n",[Name,Argc]),
    w("    while(enif_get_list_cell(env, ~s_l, &~s_h, &~s_t)) {\n", [Name,Name,Name]),
    w("        if(!~s(env, ~s_h, &~s[i++])) Badarg(~w,\"~s\");\n"
      "        ~s_l = ~s_t;\n    };\n",[enif_get(Type), Name,Name,?OP,Name,Name,Name]),
    w("  }\n", []),
    store_free({Name++"_free", Name}),
    {P,Argc+1};
decode_var(P=#arg{name=Name, in=true, alt=Alt,
    ...
    w("    ~s = (~s *) enif_alloc(~w*~s*sizeof(~s));\n",[Name, Type, TSz, Len, Type]),
    w("    ~s_l = argv[~w];\n",[Name,Argc]),
    w("    while(enif_get_list_cell(env, ~s_l, &~s_h, &~s_t)) {\n", [Name,Name,Name]),
    w("        if(!enif_get_tuple(env, ~s_h, &~s_a, &~s_tpl) || ~s_a != ~w) {\n"
      "            Badarg(~w,\"~s\");~n        } else {\n", [Name, Name, Name, Name, TSz,?OP,Name]),
    R = fun(N) ->
                w("        if(!~s(env, ~s_tpl[~w], &~s[~w])) Badarg(~w,\"~s\");\n",
                  [enif_get(Type), Name, N, Name, N, ?OP,Name])
        end,
...
decode_var(P=#arg{name=Name, in=false,
  ...
  when Base =:= binary; Base =:= string ->
    ...
    case Base of
        string ->
            w("  ~s = enif_alloc((int) ~s*sizeof(~s));\n", [Name,Sz,T]),
            store_free(Name);
        binary when T == "GLvoid" ->
...
decode_var(P=#arg{name=Name, in=false, type=#type{name=T,single={list,Sz,_}}}, Argc) ->
    w("  ~s = enif_alloc((int)~s*sizeof(~s));\n", [Name,Sz,T]),
    w("  ~s_ts = (ERL_NIF_TERM *) enif_alloc((int)~s*sizeof(ERL_NIF_TERM));\n",[Name,Sz]),
    store_free(Name),
    store_free(Name ++ "_ts"),
    {P,Argc+1};
...
build_ret(Name,_Q,#type{name=T,base=Base,size=Sz,single=true})
  when Base =:= int; Base =:= bool ->
    ...
    if Sz =< 4 -> io_lib:format("     enif_make_int(env, ~s)", [Name]);
       Ptr     -> io_lib:format("     enif_make_uint64(env, (uint64_t) ~s)", [Name]);
       true    -> io_lib:format("     enif_make_int64(env, (int64_t) ~s)", [Name])
    end;
...
build_ret(Name,false,#type{single={list,_,Sz}}) ->
    io_lib:format("     enif_make_list_from_array(env, ~s_ts, ~s)",[Name, Sz]);
...
finits([F|Fs]=Retry, Fun, Id) when Id < 6000 ->
