...
decode_arg(N,#type{name=Type,base={term,_},single=Single,mod=Mod0},Arg,Argc) ->
    case Single of
        ...
        _ ->
            ...
            w("  while(!enif_is_empty_list(env, ~sTail)) {~n", [N]),
            w("    if(!enif_get_list_cell(env, ~sTail, &~sHead, &~sTail)) ~s;~n",[N,N,N,badarg(N)]),
            w("    *~s_ptr = new ~s(~s);~n", [N,Type,Argc]),
            w("    ~s_ptr++;~n", [N]),
            w("  };~n",[]),
            store_free(N)
    end;

decode_arg(N,#type{single=array,base=Int,name=Type},Arg,Argc) when
      Int =:= int; Int =:= int64 ->
    wa("  ~s * ~s;~n",[Type,N],Arg),
    w("  ~s * ~s_ptr;~n",[Type,N]),
    case Int of
        int -> w("  int ~s_tmp;~n",[N]);
        int64 -> w("  int64 ~s_tmp;~n",[N])
    end,
...
call_arg(#param{name=N,type=#type{base={term, Class}, by_val=false, ref={pointer,2}}}) ->
    "(" ++ Class ++ " **) " ++ N;
call_arg(#param{name=N,type=#type{by_val=false, ref={pointer,2}}}) ->
    "&" ++ N;
